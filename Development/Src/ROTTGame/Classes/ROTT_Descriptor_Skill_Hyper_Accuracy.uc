...
) 
{
  ...
  switch (type) {
    case HYPER_SPAWN_CHANCE:
      ...
      break;
    case HYPER_ACCURACY_PERCENT:
      // Accuracy increase permanent
      attribute = 10 + 5 * gameInfo.playerProfile.getSpiritualProwess() / 500; 
      break;
    case HYPER_ACCURACY_PERCENT:
      // Accuracy increase by percent
...
defaultProperties 
{
  ...
  skillAttributes.add((attributeSet=GLYPH_SET,mechanicType=HYPER_ACCURACY_PERCENT,tag="%permAccuracy",font=DEFAULT_SMALL_GREEN,returnType=INTEGER));
  skillAttributes.add((attributeSet=GLYPH_SET,mechanicType=HYPER_ACCURACY_PERCENT,tag="%maxAccuracy",font=DEFAULT_SMALL_GREEN,returnType=INTEGER));
